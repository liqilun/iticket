...
public class PageUtil {
	...
	public PageUtil(int rowsCount, int rowsPerPage, int currentPage,
			String url, boolean isShowFirst, boolean isShowLast) {
		this.rowsCount = rowsCount;
		this.rowsPerPage = rowsPerPage;
		this.currentPage = currentPage;
		this.pageCount = (rowsCount - 1) / rowsPerPage + 1;
	...
	public void initPageInfo(Map params, List paramNames) {
		...
		if (isNextPage()) {
			this.nexturl = baseUrl + "?pageNo=" + (currentPage + 1)
					+ commonParam;
		}
		if (isFirstPage()) {
			if (StringUtils.isNotBlank(commonParam))
				this.firsturl = baseUrl + "?"
						+ StringUtils.substring(commonParam, 1);
			else
				this.firsturl = baseUrl;
		}
		if (isLastPage()) {
	...
	public boolean isOnLast(String pageNo) {
		if (StringUtils.isBlank(pageNo))
			pageNo = "0";
		if (isLastPage()
				&& StringUtils.equals((Integer.valueOf(pageNo) + 1) + "",
						pageCount + ""))
			return true;
		return false;
	}
